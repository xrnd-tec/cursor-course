...
| 0:20 | 第3章 三つの道具 | Tab / Cmd+K / Agent を1つずつ触る（10分） | 全員 |
...
5. サイドバーから `practice/calculator.js` を開く
...
> `Tab`、`Cmd+K`、Agent。3つを1つずつ、自分で触ります。

### ［スライド］解説

**Cursor は、VS Code 系のエディタに「コードを読んで・書いて・実行できる AI」が載ったもの**です。AI に頼む手段は3つあります。

| 手段 | 画面からやる | Mac | Windows | 用途 | 典型シーン |
|------|-------------|-----|---------|------|------------|
| **Tab** | （なし） | （自動） | （自動） | 入力中に続きを提案 | 関数の続き、import、繰り返しパターン |
| **インライン編集** | （なし） | `Cmd+K` | `Ctrl+K` | 選択範囲だけ直す | 「この関数を async に」「この文言を変えて」 |
| **Agent** | 右上の `Agents Window` | `Cmd+I` | `Ctrl+I` | 会話しながら複数ファイルを扱う | 機能追加、調査、リファクタ、バグ修正 |
...
`applyDiscount` 関数全体を選択して `Cmd+K`（Windows は `Ctrl+K`）。次を貼り付けて送ります。

```text
TODOコメントだけを、処理内容が分かる具体的な日本語に直して。コードは変更しないで。
```
...
`Cmd+I`（Windows は `Ctrl+I`）。**まだ何も送りません。** 開くだけです。

### 講師が話すこと

**①に入る前**: Mac の `Cmd` は Windows では `Ctrl` に読み替えること（`Shift+Tab` は両方同じ）。**ショートカットが効かない人は、画面のボタンから操作してよい**と先に言っておきます。受講者がキーバインドを変えていることがあり、ここでつまずくと後が続きません。
...
**③のあと**: 「Tab は『続き』、Cmd+K は『ここだけ』、Agent は『会話しながら』。今日は主に Agent を使います。」

### チェックポイント

- [ ] Tab の灰色提案を一度は見た
- [ ] `Cmd+K` で1回変更して Accept か Reject した
- [ ] Agent の入力欄が開いている
...
| `Cmd+K` が効かない | Windows は `Ctrl+K`。キーバインドを変えている人は、選択してから右クリックメニューを探す |
| 別ウィンドウが開いた | Agent が別ウィンドウで開く版がある。右上の Agent ボタンか `Option+Cmd+J` で右パネルに戻せる |
...
`Cmd+I` で入力欄を開き、左下のモード名をクリックして **Ask** を選びます（`Shift+Tab` はモードを順送りします）。
...
送ったら**差分（diff）が出るまで待ちます**。まだ Keep しないでください。次の章で読みます。
...
> **表記の注意**: 行単位の差分は **Keep / Undo**、`Cmd+K` のインライン編集だけは **Accept / Reject** と表示されます。**意味は同じ**です（旧版はどちらも Accept / Reject）。
...
- [ ] `calculator.js` の `applyDiscount` が実装されている
- [ ] Ask の答え合わせが返ってきた
...
提出は「誰が1サイクル回せたか」を巡回なしで把握するためのものです。第2回の入り方を決める材料になります。
