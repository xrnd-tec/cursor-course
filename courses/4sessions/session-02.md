...
**受講者が資料を見ながら手を動かし、講師が説明しながら進める**前提で書いてあります。時間配分もその想定です。各章は4ブロックです。
...
入力欄のモデル表示（**Auto** や **High**）をクリックするか `Cmd+/`（Windows は `Ctrl+/`）で、**Auto ではなく特定のモデル**を選びます。
...
| 詰まり | 対処 |
|--------|------|
| モデルの選び方が分からない | 入力欄のモデル名をクリック。`Cmd+/` でも開く |
| ブラウザで開けない | ファイルのパスを確認。Live Server 拡張を使う手もある |
...
| **Custom Mode** | `Option + Enter` で固定。**セッション全体** |
